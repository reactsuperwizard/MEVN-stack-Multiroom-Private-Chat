...
const checkEditProfileFields = async (req, res, next) => {
	...
	if (req.body.email) {
		...
		if (req.body.email.length > 35 || req.body.email.length < 3) {
			errors.push({
				param: 'email',
				msg: 'Input email with 3 ~ 35 characters',
			});
		}
	}
	...
	if (req.body.handle) {
		...
		if (req.body.handle.length > 35 || req.body.handle.length < 3) {
			errors.push({
				param: 'handle',
				msg: 'Input handle with 3 ~ 35 characters',
			});
		}
	}

	if (req.body.location) {
		if (req.body.location.length > 35 || req.body.location.length < 3) {
			errors.push({
				param: 'location',
				msg: 'Input location with 3 ~ 35 characters',
			});
		}
	}
	if (req.body.bio) {
		if (req.body.bio.length > 100 || req.body.bio.length < 3) {
			errors.push({
				param: 'Bio',
				msg: 'Input about your self with 3 ~ 100 characters',
			});
		}
	}
